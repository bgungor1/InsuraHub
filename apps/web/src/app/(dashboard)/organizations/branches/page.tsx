...
export default function BranchesPage() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useBranchesQuery({
    ...
    <div className="flex flex-col h-[calc(100vh-7rem)] space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-foreground">Şubeler & Havuzlar</h1>
            <p className="text-xs text-muted-foreground">
              Acentelere bağlı poliçe üretim şubeleri ve komisyon havuzları
            </p>
          </div>
        </div>
